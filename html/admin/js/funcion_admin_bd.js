$( document ).ready(function() {
    ...
    $("#boton_modifica_mencion").click(function(){
        $.ajax({
            ...
            type: 'POST',
            async: true,
            data: 'mencion=BD&descrip='+$("#descripcion_mencion").val(),
            success: function(datos_recibidos) {
    ...
    $("#boton_agregar_ramo").click(function(){
        ...
        $.ajax({
            ...
            type: 'POST',
            async: true,
            data: 'name='+name.val()+'&corta='+$("#descrip_corta_nueva option:selected").text()+'&larga='+larga.val()+'&profe='+profe.val()+'&mencion=BD&code='+corta.val()+'&urlEnvio='+urlSend[urlSend.length-1],
            success: function(datos_recibidos) {
                if(datos_recibidos=="ok"){
            ...
            $(".boton_borrar").click(function(){
                ...
                $("#boton_aprobar_borrar").click(function(){
                    $.ajax({
                        ...
                        type: 'POST',
                        async: true,
                        data: 'mencion=BD&code='+data[2]+'&nombre_ramo='+$("#nombre_"+data[1]).text(),
                        success: function(datos_recibidos) {
                            if(datos_recibidos == "ok"){
            ...
            $(".boton_mostrar").click(function(){
                ...
                $("#boton_aprobar_mostrar").click(function(){
                    $.ajax({
                        ...
                        type: 'POST',
                        async: true,
                        data: 'mencion=BD&code='+data[2]+'&tipo='+data[3]+'&nombre_ramo='+$("#nombre_"+data[1]).text(),
                        success: function(datos_recibidos) {
                            if(datos_recibidos == "ok"){
                                if($("#estado_"+data[1]).text() == "Visible"){
